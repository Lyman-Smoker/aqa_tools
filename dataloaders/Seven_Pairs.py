...
class Seven_Pairs(torch.utils.data.Dataset):
    def __init__(self, transform, class_idx_list, data_root, frame_length, subset,score_range=100, num_exemplar=1, args=None):
        random.seed(0)
    ...
    def delta(self):
        ...
        for i in range(len(dataset)):
            for j in range(i+1,len(dataset)):
                delta.append(
                    abs(
                    normalize(dataset[i][2], self.class_idx_list[0], self.score_range) -
                    normalize(dataset[j][2], self.class_idx_list[0], self.score_range)))
        return delta