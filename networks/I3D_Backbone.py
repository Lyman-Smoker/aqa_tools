...
class I3D_backbone(nn.Module):
    ...
    def forward(self, target, exemplar=None):
        ...
        if exemplar is None:
            video_pack = torch.cat([target[:, :, i: i + 16] for i in start_idx])  # 10*2N, c, 16, h, w
            total_feature = self.backbone(video_pack).reshape(10, len(target), -1).transpose(0, 1)  # 2N * 10 * 1024
            total_feature = total_feature.mean(1)
            return total_feature
        else:
            total_video = torch.cat((target, exemplar), 0)  # 2N C H W
            video_pack = torch.cat([total_video[:, :, i: i + 16] for i in start_idx])
            total_feature = self.backbone(video_pack).reshape(10, len(total_video), -1).transpose(0, 1)
...
if __name__ == '__main__':
    ...
    input = torch.randn((1, 3, 103, 224, 224))
